...
dirpath = '/projects/TDataFus/gyzhao/TF/data/MOPITT/MOP01.006'
...
for m in range(1, 13):
    for d in range(1, 31):
        ...
        if not path.exists(inPath):
            exist[ 'MOP01.006' + '/' + dateStr] = 'File Doesnot Exist'
        else:
            for fname in os.listdir(inPath):
                ...
                if fname.endswith('.hdf'):
                    ...
                    if statinfo.st_size == 0:
                        exist['MOP01.006' + '/' + dateStr] = 'Exists but Empty'
                    else:
                        exist['MOP01.006' + '/' + dateStr] = 'Exists and not Empty'


#special cases for the months with 31st    
if not path.exists(os.path.join(dirpath, '2013.01.31')):
    exist[ 'MOP01.006' + '/' + '2013.01.31'] = 'File Doesnot Exist'

if not path.exists(os.path.join(dirpath, '2013.03.31')):
    exist[ 'MOP01.006' + '/' + '2013.03.31'] = 'File Doesnot Exist'

if not path.exists(os.path.join(dirpath, '2013.05.31')):
    exist[ 'MOP01.006' + '/' + '2013.05.31'] = 'File Doesnot Exist'

if not path.exists(os.path.join(dirpath, '2013.07.31')):
    exist[ 'MOP01.006' + '/' + '2013.07.31'] = 'File Doesnot Exist'

if not path.exists(os.path.join(dirpath, '2013.08.31')):
    exist[ 'MOP01.006' + '/' + '2013.08.31'] = 'File Doesnot Exist'

if not path.exists(os.path.join(dirpath, '2013.10.31')):
    exist[ 'MOP01.006' + '/' + '2013.10.31'] = 'File Doesnot Exist'

if not path.exists(os.path.join(dirpath, '2013.12.31')):
    exist[ 'MOP01.006' + '/' + '2013.12.31'] = 'File Doesnot Exist'

#deleting extra days from the month of Feb
del exist['MOP01.006' + '/' + '2013.02.29']
del exist['MOP01.006' + '/' + '2013.02.30']

file = open("MOPITT_missing.txt", "w").close()
file = open("MOPITT_missing.txt", "w")
for i in exist:
    if exist[i] == 'File Doesnot Exist' or exist[i] == 'Exists but Empty':
        file.write(i + " " + exist[i] + "\n")
